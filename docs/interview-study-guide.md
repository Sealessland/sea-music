...
### 6.4 `sync.Mutex` 与并发安全组件在项目中的选择

S3 下载 URL 缓存用普通 `sync.Mutex`，见 `internal/video/objectstore.go`；指标则交给并发安全的 Prometheus Counter/Histogram/Collector，不再维护自研的 `map + RWMutex` 聚合器。

- URL 缓存读命中也涉及“检查过期”这一组合操作，且实现追求简单；普通 Mutex 足够。
- Prometheus 客户端负责并发更新、采集和文本格式协商，业务代码只选择指标类型与低基数标签。
...
普通 Go map 不支持并发读写；并发写或读写可能触发 race，甚至运行时错误。S3 URL cache 的 map 所有访问都在 Mutex 下；HTTP 与限流指标使用 Prometheus 客户端提供的并发安全指标类型。
...
项目统一使用 Gin 注册路由与 handler，不再通过适配器转回 `net/http` handler。中间件包含结构化请求日志、统一 panic recovery、安全头、`gin-contrib/cors`、request ID 与 OTel，各路由再挂认证和限流；严格 JSON 绑定基于 Gin binding，并额外拒绝多 JSON 值。
...
项目 HTTP 指标使用 Gin 的 `FullPath`，如 `/api/v1/videos/:video_id`，而不是实际 URL；label 只有 method、route、status_class。具体 video ID 适合日志或 trace attribute，不适合常驻指标 label。

当前指标由官方 Prometheus Go client 的 Counter、Histogram 和 Collector 聚合并由 `promhttp` 暴露：
