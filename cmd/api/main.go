...
func run() error {
	...
	rateMiddleware := appapi.NewRateLimitMiddleware(rateLimiter, rateMetrics, logger)
	eventRepository := events.NewPostgresRepository(database)
	rateMiddleware.WithEventBacklog(eventRepository)
	rateMiddleware.WithCounterReconciliation(social.NewCounterReconciler(database, redisClient))
	rateMiddleware.WithRuntimePools(database, redisClient)
	...
	handler := httpserver.NewHandlerWithOrigins(logger, readiness, cfg.HTTP.AllowedOrigins, appapi.RegisterFrontendRoutes, identityHandler.RegisterRoutes, videoHandler.RegisterRoutes, socialHandler.RegisterRoutes, discoveryHandler.RegisterRoutes, eventsHandler.RegisterRoutes, rateMiddleware.RegisterRoutes)
