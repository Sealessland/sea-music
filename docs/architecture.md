...
3. Dispatcher 只有收到所选 broker 的持久化 ack 后才确认 Outbox；Worker 通过统一 Consumer 契约消费并使用 Inbox 去重。
4. Worker 运行真实 ffprobe/ffmpeg，将视频推进审核态，并原子写入 `video.ready_for_moderation` Outbox。
5. 专用事件 consumer 在 Inbox 事务内只创建 moderation dispatch job；网络调用在事务外由租约循环执行，通过 gRPC `StartReview/GetReview` 启动和回收长任务。
...
- 消息 broker 是 API 的可选 readiness 依赖，已有写请求仍可原子进入 Outbox；数据库、Redis 和对象存储是必需依赖。
